...
public class GameController {
	...
	public void displayCount(int wordCount) { //Method that sets the labels based on word count
		countLabel.setText(
			"Spell word " +
			Integer.toString(wordCountCurrent) +
			" of " +
	...
	public void deleteWord() {
		try {
			String delWord =
				...
				uniqueFileTemp +
				" " +
				uniqueFile; //Deletes the first word of the file
			Bash.exec(delWord);
			answerField.clear();
		} catch (Exception f) {
			f.printStackTrace();
			...
			if (reader.hasNextLine()) {
				...
				if (answer.equals(word)) { //Compares the answer with the value from text field, and reads out the correctness
					...
					if (!prevWord.equals(word)) { //If user has only gotten it wrong once, read the word again and wait for answer
						prevWord = word;
						statusLabel.setText(
							"Incorrect, try once more. Hint: " + word.charAt(1)
						);
						String readWord =
							"echo Incorrect, try once more " + word + ", " + word;
